<div class="row col-12">
    <div class="mb-2">
        <button class="button-plus-icon"><i class='plus-icon bx bxs-plus-circle'></i></button>
    </div>
    <div class="table-container">
        <table id="example" class="table table-responsive table-stripped ">
            <thead class="">
                <tr class="background-secondary">
                    <th scope="col" class="text-white">CODE</th>
                    <th scope="col" class="text-white">TITLE</th>
                    <th scope="col" class="text-white">CATEGORY</th>
                    <th scope="col" class="text-white">TYPE</th>
                    <th scope="col" class="text-white">VISIBILITY</th>
                    <th scope="col" class="text-white">STATUS</th>
                    <th scope="col" class="text-white">ACTIONS</th>
                </tr>


                <tr class="background-grey">
                    <td><input type="search" name="" class="form-control table-search-bar"
                            placeholder="Search Code " id="column-1">
                    </td>
                    <td><input type="search" name="" class="form-control table-search-bar"
                            placeholder="Search Title " id="column-2">
                    </td>
                    <td><input type="search" name="" class="form-control table-search-bar"
                            placeholder="Search Category " id="column-3">
                    </td>
                    <td><input type="search" name="" class="form-control table-search-bar"
                            placeholder="Search Type " id="column-4">
                    </td>
                    <td><input type="search" name="" class="form-control table-search-bar"
                            placeholder="Search Visibility " id="column-5">
                    </td>
                    <td><input type="search" name="" class="form-control table-search-bar"
                            placeholder="Search Status " id="column-6">
                    </td>
                    <td>
                    </td>
                </tr>

            </thead>
            <tbody>

                <tr>
                    <td>JAVA-A001</td>
                    <td>Hello World</td>
                    <td>Program</td>
                    <td>Context</td>
                    <td>Private</td>
                    <td><span class="badge bg-success">published</span></td>
                    <td>
                        <div class="dropdown">
                            <input class=" dropdown-toggle action-toggle" value="Actions" readonly
                                data-toggle="dropdown" onclick="toggleDropdown(this)">

                            <div class="dropdown-menu">
                                <a class="dropdown-item" href="#" onclick="performAction('View', this)">View</a>
                                <a class="dropdown-item" href="#" onclick="performAction('Edit', this)">Edit</a>
                                <a class="dropdown-item" href="#"
                                    onclick="performAction('Delete', this)">Delete</a>
                            </div>
                        </div>
                    </td>
                </tr>
                <tr>
                    <td>JAVA-A001</td>
                    <td>Hello World</td>
                    <td>Program</td>
                    <td>Context</td>
                    <td>Private</td>
                    <td><span class="badge bg-success">published</span></td>
                    <td>
                        <div class="dropdown">
                            <input class=" dropdown-toggle action-toggle" value="Actions" readonly
                                data-toggle="dropdown" onclick="toggleDropdown(this)">

                            <div class="dropdown-menu">
                                <a class="dropdown-item" href="#" onclick="performAction('View', this)">View</a>
                                <a class="dropdown-item" href="#" onclick="performAction('Edit', this)">Edit</a>
                                <a class="dropdown-item" href="#"
                                    onclick="performAction('Delete', this)">Delete</a>
                            </div>
                        </div>
                    </td>
                </tr>

                <tr>
                    <td>JAVA-A001</td>
                    <td>Hello World</td>
                    <td>Program</td>
                    <td>Context</td>
                    <td>Private</td>
                    <td><span class="badge bg-success">published</span></td>
                    <td>
                        <div class="dropdown">
                            <input class=" dropdown-toggle action-toggle" value="Actions" readonly
                                data-toggle="dropdown" onclick="toggleDropdown(this)">

                            <div class="dropdown-menu">
                                <a class="dropdown-item" href="#" onclick="performAction('View', this)">View</a>
                                <a class="dropdown-item" href="#" onclick="performAction('Edit', this)">Edit</a>
                                <a class="dropdown-item" href="#"
                                    onclick="performAction('Delete', this)">Delete</a>
                            </div>
                        </div>
                    </td>
                </tr>

                <tr>
                    <td>JAVA-A001</td>
                    <td>Hello World</td>
                    <td>Program</td>
                    <td>Context</td>
                    <td>Private</td>
                    <td><span class="badge bg-success">published</span></td>
                    <td>
                        <div class="dropdown">
                            <input class=" dropdown-toggle action-toggle" value="Actions" readonly
                                data-toggle="dropdown" onclick="toggleDropdown(this)">

                            <div class="dropdown-menu">
                                <a class="dropdown-item" href="#"
                                    onclick="performAction('View', this)">View</a>
                                <a class="dropdown-item" href="#"
                                    onclick="performAction('Edit', this)">Edit</a>
                                <a class="dropdown-item" href="#"
                                    onclick="performAction('Delete', this)">Delete</a>
                            </div>
                        </div>
                    </td>
                </tr>

                <tr>
                    <td>JAVA-A001</td>
                    <td>Hello World</td>
                    <td>Program</td>
                    <td>Context</td>
                    <td>Private</td>
                    <td><span class="badge bg-success">published</span></td>
                    <td>
                        <div class="dropdown">
                            <input class=" dropdown-toggle action-toggle" value="Actions" readonly
                                data-toggle="dropdown" onclick="toggleDropdown(this)">

                            <div class="dropdown-menu">
                                <a class="dropdown-item" href="#"
                                    onclick="performAction('View', this)">View</a>
                                <a class="dropdown-item" href="#"
                                    onclick="performAction('Edit', this)">Edit</a>
                                <a class="dropdown-item" href="#"
                                    onclick="performAction('Delete', this)">Delete</a>
                            </div>
                        </div>
                    </td>
                </tr>
                <tr>
                    <td>JAVA-A001</td>
                    <td>Hello World</td>
                    <td>Program</td>
                    <td>Context</td>
                    <td>Private</td>
                    <td><span class="badge bg-success">published</span></td>
                    <td>
                        <div class="dropdown">
                            <input class=" dropdown-toggle action-toggle" value="Actions" readonly
                                data-toggle="dropdown" onclick="toggleDropdown(this)">

                            <div class="dropdown-menu">
                                <a class="dropdown-item" href="#"
                                    onclick="performAction('View', this)">View</a>
                                <a class="dropdown-item" href="#"
                                    onclick="performAction('Edit', this)">Edit</a>
                                <a class="dropdown-item" href="#"
                                    onclick="performAction('Delete', this)">Delete</a>
                            </div>
                        </div>
                    </td>
                </tr>
                <tr>
                    <td>JAVA-A001</td>
                    <td>Hello World</td>
                    <td>Program</td>
                    <td>Context</td>
                    <td>Private</td>
                    <td><span class="badge bg-success">published</span></td>
                    <td>
                        <div class="dropdown">
                            <input class=" dropdown-toggle action-toggle" value="Actions" readonly
                                data-toggle="dropdown" onclick="toggleDropdown(this)">

                            <div class="dropdown-menu">
                                <a class="dropdown-item" href="#"
                                    onclick="performAction('View', this)">View</a>
                                <a class="dropdown-item" href="#"
                                    onclick="performAction('Edit', this)">Edit</a>
                                <a class="dropdown-item" href="#"
                                    onclick="performAction('Delete', this)">Delete</a>
                            </div>
                        </div>
                    </td>
                </tr>
            </tbody>
        </table>
    </div>
    <div class="col-md-4">
    </div>
    <div class="col-md-4">
        <div id="pagination" class="d-flex justify-content-center mt-4"></div>
    </div>
</div>
